...
## 追記（2026-08-29）：`status` をレビューの理解に合わせた

「レビュー後に変えたもの」で、`status` の意味がレビュー時の説明（既読と未読）から配信状況に変わっていると記した。
**実装のほうをレビューの理解に戻した。**

未読の行だけを画面に出し、チェックのボタンで既読にする。
経緯と、配信状況の列を足さない理由は `20260817-push-notification.md` の改訂（2026-08-29）にある。

drawSQL の `status` 列コメントは、前の追記で「配信を依頼できたか。sent または failed。既読ではない」に直したばかりだった。
次のエクスポートで「既読状態。unread または read」に改める。

## 追記（2026-08-29）：`status` の列コメントを直した

直前の追記で予告した書き換えを済ませた。
drawSQL 側と `doc/specifications/drawSQL-pgsql-export-2026-08-28.sql` の両方で「既読状態。unread または read」になっている。

エクスポートは出力し直さず、ファイルを直接編集した。
既読状態への変更は列の型も制約も動かしておらず（8-29 のマイグレーションはデータの書き換えだけである）、差分がコメント1行に収まるためである。
ファイル名の日付も 8-28 のままにした。
