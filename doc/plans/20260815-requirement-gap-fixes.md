...
「判断待ち」に挙げていたカテゴリのCRUD画面について、**画面を足す**と決まった。サーバ側を消す案は採らない。消すと、カテゴリを1つ増やすたびにシーダーの修正とデプロイが必要になり、マスタ管理画面として中途半端になる。

絞り込みのサイドバーには手を入れず、ヘッダーから開くダイアログに分けた。同じ部品に絞り込みのチェックボックスと編集のUIが同居すると、チェックを付けたいのか名前を直したいのかが混ざる。管理画面には `PasswordChangeDialog` の実装が既にあり、作りをそのまま流用できる。

### サーバ側で足りなかったもの

CRUDの4操作は揃っていたが、画面を作ると3点足りなかった。

1. **カテゴリの商品件数**。削除できるかどうかの表示に使う。カテゴリを props に渡しているのは `CategoryController` ではなく `ProductController::index` なので、そこに `withCount('products')` を足す
2. **名前の重複を防ぐ検証**。同名のカテゴリをいくつでも作れる状態だった。絞り込みのサイドバーに同じ名前が並び、`CategoryIcon` のアイコン対応も名前で引いているため実害がある。`update` は自分自身を `ignore($category->id)` で除外しないと、名前を変えずに保存したときに自分と衝突して弾かれる
3. **`CategoryController::index` の削除**。ルートから呼ばれておらず、`admin/dashboard` を `categories` だけ渡して描画するため、呼ばれると `products` prop が欠けて画面が壊れる

`products_count` はフロントの `AdminCategory` 型に持たせ、`Category` 自体には足さない。絞り込みのサイドバーは件数を必要としない。`AdminProduct` と `StockFormProduct` と同じ扱いである。

### 検証メッセージが日本語化されていなかった

重複した名前で保存すると、画面に `validation.unique` というキーがそのまま出た。`APP_LOCALE=ja` に対して `lang` ディレクトリが無く、Laravel 11以降は英語の翻訳しか内蔵しないため、キーが未解決のまま返る。`APP_FALLBACK_LOCALE` も `ja` なので英語にも落ちない。

`lang/ja` に `validation.php`、`auth.php`、`passwords.php` を置いた。これは管理画面だけの問題ではなく、新規登録やストック設定を含む全画面の検証メッセージに効く。ログイン失敗の `auth.failed` とパスワード再設定の各状態も同じ理由で壊れていた。

`validation.php` の `attributes` で属性名も日本語にする。これが無いと「その name は既に使われています」となる。`name` はカテゴリ名、商品名、ユーザ名で共用しているため「名前」に留めた。画面ごとに言い回しを変えたい場合は、その `validate()` の第2引数で個別にメッセージを渡すことになる。

`.env` の `APP_FALLBACK_LOCALE` は `ja` のまま残している。`validation.php` に無い規則を使うとキーが生で出るため、`en` にして英語へ落とす選択肢もある。日本語と英語が混ざるのを避けるか、キーが出るのを避けるかの好みで決まる。

### 残る判断待ち

パスワード再設定の方式のみ。要件は仮パスワードをメールで送る形、実装は Fortify 標準のトークン付きリンク。

## 改訂（2026-08-17・解消した項目）

この計画書を残課題の一覧として読むと誤るところが2つあるので、現状を記す。

**管理者のパスワードのハッシュ化は済んでいる。** 「5. 初期データ」に「`Admin` モデルは `password` に `hashed` のキャストを持たず」「別途直す価値がある」と書いたが、f84ab8f で解消した。`app/Models/Admin.php` にキャストがあり、`AdminSeeder` も `AuthController::updateAdminPassword` も生の値を渡す。`User` と同じ扱いになっている。

**パスワード再設定の方式は決着した。** 要件どおりの仮パスワード方式に切り替えた（PR #10）。経緯と実装は `20260816-temporary-password-reset.md` にある。

残っているのは Push通知の実体（`20260729-pwa-general-user-only.md` のフェーズ2と3）と、この計画書の「残件」に挙げたテスト一式である。

## 追記（2026-08-17）：Push通知の参照先が変わった

上とスコープ表（1行目）と「残件」で `20260729-pwa-general-user-only.md` のフェーズ2と3を指している箇所は、**`20260817-push-notification.md` に読み替える。** フェーズ1（PWA化）が完了したため、購読と配信をそちらへ切り出した。移管元は今後 PWA の骨格だけを扱う。

## 改訂（2026-08-23）：ログ設計は実施する

「対象外と決めたこと」で非機能要件の3項目をまとめて実施しないとしたが、このうち**ログ設計は実施する**方針に改めた。
設計は `20260823-request-error-logging.md` にある（2026-08-23 時点で実装は未着手）。

Sanctum認証と REST API化は据え置く。
この2つは既存画面すべてに波及するのに対し、ログの記録はミドルウェアと設定の追加で収まり、画面にも認証にも影響しない。

## 追記（2026-08-29）：要件定義書の側を実装に合わせた

「対象外と決めたこと」で Sanctum認証と REST API化を実施しないと決めたが、`AGENTS.md` の非機能要件は元の記述のままだった。
決定から2週間、要件だけが残り、読む人には未実装に見える状態が続いていた。

`AGENTS.md` の該当2項目を取り消し線と決定の併記に改めた。
...
行を消さずに残したのは、なぜ Sanctum を使っていないのかが分からなくなるためである。
理由と本書への参照を1つの節にまとめて添えた。

ログ設計は実施すると決めているので、記述をそのまま残す（改訂 2026-08-23）。
