...
## 追記（2026-08-29・続き）：要件定義書に実装との差を書き足した

認証とAPI設計に続き、`AGENTS.md` の機能要件と非機能要件を実装に合わせた。
本書が扱ってきた「要件との差分」の最終的な行き先が、計画書ではなく要件定義書そのものになる。

### 実装が要件より狭いところ

| 箇所 | 書き足した内容 |
|---|---|
| ダッシュボードの購入予定品 | 1週間以内に予定日が来るものだけ表示する。件数の表示も同じ範囲 |
| Push通知の履歴 | 未読のものだけ表示する |
| ログ設計 | 未実装であることと設計の置き場 |

カレンダーには1週間の絞り込みを掛けない。
先の予定が見えなくなると月単位で見せる意味が無い。
要件は「次回購入日とカレンダー」と「購入予定品」を別の項目として書いており、絞り込みの範囲を揃える必要はない。

### 実装が要件より広いところ

| 箇所 | 書き足した内容 |
|---|---|
| ストック管理 | 同じ商品を重ねて登録できない。編集では商品を変更できない |
| ダッシュボード | 購入ボタンで購入済みを記録する |
| Push通知 | 購入予定日を過ぎたものにも毎朝通知する。止まるのは購入ボタンだけである |
| Push通知の設定 | ON/OFFの正本は端末が持つ |
| 管理者用マスタ画面 | カテゴリのCRUD。使用中の商品とカテゴリは削除できない |
| 非機能要件 | PWA対応 |

要件に無い機能を足したときは、要件定義書の側に追記する。
実装だけが先に進むと、次に読む人には要件と実装のどちらが古いのか分からない。

### 触れなかったもの

「5. 主要な画面構成・一部」に設定画面が載っていない。
節の題が「一部」であり、網羅を意図していない。
