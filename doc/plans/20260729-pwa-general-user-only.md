...
## 確定（2026-08-29）：`VERSION` はデプロイのたびに上げる

`public/app/sw.js` の `VERSION` を、デプロイのたびに手で上げると決めた。
`ASSET_CACHE` と `SHELL_CACHE` の名前がこの値から作られ、`activate` が名前の違う古いキャッシュを消すので、上げれば前のビルドの成果物が残らない。

`/build/assets/` を stale-while-revalidate に変える案は採らない。
ビルド成果物はファイル名に内容ハッシュを持つので、内容が変われば別のURLになる。
同じURLの中身が変わることは無く、取得し直す意味がない。

### 忘れると静かに壊れる

この手順は、忘れても手元では気付けない。
古い成果物を握ったブラウザだけが古いコードで動き、新しいサーバの応答と噛み合わずに落ちる。
開発中に2回起きており、`n.map is not a function` のように、原因から遠い場所でエラーが出る。

デプロイの手順を作るときに、この1行を組み込む必要がある。
`VERSION` をビルド時に埋める形にすれば手順から外せるが、`sw.js` は Vite の処理対象外（`tsconfig.json` の `include` にも eslint の対象にも入っていない）なので、埋めるための仕組みを別に用意することになる。
デプロイ先が決まっていない今は判断できないため、手で上げる前提で置く。

### 開発中の回避

`npm run build` し直しても古い画面が出るときは、DevTools の Application から Service Workers を Unregister し、Storage を Clear site data してからリロードする。
Application の Service Workers にある「Update on reload」を有効にしておくと起きにくい。
