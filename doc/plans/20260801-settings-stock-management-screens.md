...
# 確定（2026-08-23）：「確認したいこと」の残り2件

初版末尾の「確認したいこと」5項目のうち、1（消費日数の補正ルール）は `20260815-requirement-gap-fixes.md` が式を決めて実装し、3（保存ボタンを2つに分ける件）は改訂（2026-08-01）で取り下げた。
残っていた2と5に回答する。**これで未確定事項は無くなる。**

## 2. 世帯人数が未設定の利用者

**ストック追加画面で案内を出す。**

要件の新規登録は「ユーザ名 / メールアドレス / パスワード」であり、世帯人数の欄を足すと要件から外れる。
かといって放置すると、要件が求める「世帯人数を元に初期値を設定する」が、設定画面にたどり着かない利用者には働かない。

案内は `household_size` が `null` のときだけ、ストック追加画面（新規のとき）に出す。
設定画面へのリンクを添える。

編集のときは出さない。
消費日数の欄には保存済みの値が入っており、補正の出番がない。

世帯人数は共有プロパティの `auth.user` から読めるので、props を足す必要はない。
ダッシュボードなど他の画面には出さない。
補正が効くのはストックを新しく作るときだけで、関係のない画面で設定を促しても操作の邪魔になる。

## 5. ストック編集での商品の変更

**現状のまま、編集では商品を変えられないようにする。**

商品を差し替えたストックは、消費日数も次回購入予定日も別のものになる。
同じ行を使い回す意味が薄く、削除して作り直すのと結果が変わらない。
編集画面には削除ボタンがあるので、間違えて登録したときの逃げ道は用意されている。

要件も「商品ごとに消費日数とストック数を登録」としか言っておらず、商品の差し替えを求めていない。

`StockController::update` が `product_id` を受けない状態と、選択欄の `disabled={isEdit}` は、どちらもこの判断によるものとして残す。
